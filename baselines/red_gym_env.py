...
class RedGymEnv(Env):
    ...
    def __init__(
        ...
        self.base_explore_reward = self.explore_weight * 0.005
        self.post_explore_reward = self.explore_weight * 0.01

        self.recent_coords = []
        self.stuck_penalty = 0
        self.new_coord = False
        self.new_coord_chain = 0
        self.prev_battle_turn = 0
        self.num_battles = 0
        self.seen_maps = {}
        self.map_penalty = 0
        self.prev_map_n = 40
    ...
    def render(self, reduce_res=True, add_memory=True, update_mem=True):
        ...
        # when all params are true, this is progress bars for the rewards
        # plus reduced resolution for previous frames (so that the agent knows what has come before)

        # Shape: (144, 160, 3)
        # means: 144 rows, 160 columns, 3 values in each cell
        # each cell represents the rgb value of that pixel
        # Example elements (first 4 rows):
        # [[[R1, G1, B1] [R2, G2, B2] ... [R8, G8, B8]]  # Row 1
        #  [[R9, G9, B9] [R10, G10, B10] ... [R16, G16, B16]]  # Row 2
        #  [[R17, G17, B17] [R18, G18, B18] ... [R24, G24, B24]]  # Row 3
        #  [[R25, G25, B25] [R26, G26, B26] ... [R32, G32, B32]]]  # Row 4
        game_pixels_render = self.screen.screen_ndarray()
        if reduce_res:
            # reduce the resolution of the screen pixels
            game_pixels_render = (255*resize(game_pixels_render, self.output_shape)).astype(np.uint8)
            if update_mem:
                # set the most recent frame in memory to the current one
    ...
    def step(self, action):
        # main function used to take action + observe results

        # press a button
        self.run_action_on_emulator(action)
        self.append_agent_stats(action)

        # view the screen (and previous frames)
        self.recent_frames = np.roll(self.recent_frames, 1, axis=0)
        obs_memory = self.render()

        # update what we know about the game state
        if self.use_screen_explore:
            ...
            # print(self.recent_coords)

        self.update_heal_reward()
        self.party_size = self.read_m(PARTY_SIZE_ADDRESS)

        # calculate rewards for this action
        ...
        self.recent_memory[0, 0] = min(new_prog[0] * 64, 255) # sum of team level
        self.recent_memory[0, 1] = min(new_prog[1] * 64, 255) # % of team health
        self.recent_memory[0, 2] = min(new_prog[2] * 128, 255) # exploration
    ...
    def update_seen_coords(self):
        ...
        # once it is over a threshold
            # turn it into a repel point
            # keep track of all repel points


        #if self.new_coord and len(self.seen_coords) > prev_num_coords:
        #    self.new_coord_chain += 1

        #if len(self.seen_coords) > prev_num_coords:
        #    self.new_coord = True
        #else:
        #    self.new_coord = False
    ...
    def update_reward(self):
        # returns the total step reward plus the individual 3 rewards that make up the total for the step
        # compute reward
        ...
        return (new_step, # total reward for this step
                   (
                    new_prog[0]-old_prog[0], # team level sum reward for this step
                    new_prog[1]-old_prog[1], # % of team HP reward for this step
                    new_prog[2]-old_prog[2]  # explore reward for this step
                    )
               )
    ...
    def create_exploration_memory(self):
        # creates progress bars to represent each reward status (level, hp, exploration)
    ...
    def save_and_print_info(self, done, obs_memory):
        if self.print_rewards:
            ...
            # prog_string += f' coord chain: {self.new_coord_chain}'
    ...
    def get_explore_reward(self):
        # calculates reward for exploration
        # if the party is at a certain level, exploration reward is higher

        cur_size = self.knn_index.get_current_count() if self.use_screen_explore else len(self.seen_coords)

        if self.levels_satisfied:
            base = self.base_explore * self.base_explore_reward
            post = cur_size * self.post_explore_reward
        else:
        ...
        # self.map_penalty += self.same_map_step * 0.001
    ...
    def get_battles_reward(self):
        ...
        if self.prev_battle_turn == 0 and battle_turn == 1:
            self.num_battles += 1

        self.prev_battle_turn = battle_turn

        return self.num_battles * 0.1 * 0 #disabled
    ...
    def get_game_state_reward(self, print_stats=False):
        ...
        state_scores = {
            ...
            'battles': self.reward_scale * self.get_battles_reward(),
